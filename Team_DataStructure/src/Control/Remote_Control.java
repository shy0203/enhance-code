...
public class Remote_Control extends Control_Structure {
	...
	Scanner scanNum;	// 실험번호 입력하기 위한 스캐너 변수
	int researchNum = -1;	// 입력받은 실험번호를 담아 리턴할 변수
	...
	// 필요한 객체 생성을 위한 변수
	ClassList_Control classC;
	Paging_Control pagingC;
	Data_Experiment e;
	
	public Remote_Control() {
		// 순서대로 정리된 클래스리스트와 리스트길이를 가져옴
		classC = new ClassList_Control();
		classes = classC.getClass("Lists");
		length =  classC.getLength();
		// 페이징과 실험에 필요한 객체 생성
	...
	protected void insertNum() {
		...
		do{
			...
			if(researchNum < 0 || researchNum > length - 1){	// 입력 가능 숫자범위 조건 불만족 예외처리
	...
	public void experiment_select() {
		...
		try {
			...
			copyList = (List<Integer>) classes[researchNum].newInstance();
			// 실험부
